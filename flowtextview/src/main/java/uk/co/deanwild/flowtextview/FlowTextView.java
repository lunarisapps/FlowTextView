...
public class FlowTextView extends RelativeLayout {
        ...
        for(int block_no = 0; block_no <= blocks.length - 1; block_no++) // at the highest level we iterate through each 'block' of text
        {
            ...
            if(thisBlock.length() <= 0) { //is a line break
                ...
                while(thisBlock.length() > 0) { // churn through the block spitting it out onto seperate lines until there is nothing left to render
                    lineIndex++; // we need a new line
                    yOffset = getPaddingTop() + lineIndex * lineHeight -
                              (getLineHeight() + mTextPaint.getFontMetrics().ascent); // calculate our new y position based on number of lines * line height
                    Line thisLine = CollisionHelper.calculateLineSpaceForGivenYOffset(yOffset, lineHeight, mViewWidth,
                                                                                      obstacles); // calculate a theoretical "line" space that we have to
                    // paint into based on the "obstacles" that exist at this yOffset and this line height - collision detection essentially
                    xOffset = thisLine.leftBound;
                    maxWidth = thisLine.rightBound - thisLine.leftBound;
                    float actualWidth;
                    
                    // now we have a line of known maximum width that we can render to, figure out how many characters we can use to get that width taking
                    // into account html funkyness
                    do {
    ...
    private void readAttrs(Context context, AttributeSet attrs) {
        int[] attrsArray = new int[]{android.R.attr.lineSpacingExtra, // 0
    ...
    private int findBoxesAndReturnLowestObstacleYCoord() {
        ...
        for(int i = 0; i < childCount; i++) {
            ...
            if(child.getVisibility() != View.GONE) {
                ...
                obstacle.bottomRightx = obstacle.topLeftx + layoutParams.leftMargin + child.getWidth() +
                                        layoutParams.rightMargin; // padding should probably be included as well
